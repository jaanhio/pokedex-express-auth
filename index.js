...
app.post('/user/login', (req, resp) => {
  ...
  jsonfile.readFile(userFILE, (err, obj) => {
    let userArr = obj.user;
    let loginUser = userArr.find(user => {
      return user.username === username;
    });
    bcrypt.compare(inputPW, loginUser.password).then((res) => {
      if (res === true) {
        resp.cookie('login', true);
        resp.redirect('/');
      }
      else {
        resp.render('login', {msg: 'Incorrect email/password entered'});
      }
    });
  });
});
...
app.post('/user/new', (req, res) => {
  console.log(req.body);
  let unHashedPW = req.body.password;
  let username = req.body.username;
  // encrypt user's PW
  bcrypt.hash(unHashedPW, 1, (err, hash) => {
    let hashedPW = hash;
    jsonfile.readFile(userFILE, (err, obj) => {
      let userArr = obj.user;
      let newUser = {
      ...
      res.cookie('login', loginStatus);
      jsonfile.writeFile(userFILE, user, (err) => {
        res.redirect('/');
      });
    });
  });
...
app.get("/", (req, res) => {
  ...
  // for sorting pokemon list in numerical order based on pokemon id 
    ...
    jsonfile.readFile(FILE, (err, obj) => {
      ...
      // res.render("home", { pokeNameArr: pokeNameArr, loginStatus: loginStatus });
    });
  }
});
